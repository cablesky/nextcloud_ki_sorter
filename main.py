...
def normalize_nextcloud_inbox_path(nc_file_path: str, user: Optional[str] = None) -> str:
...
def process_file_pipeline(nc_file_path: str, user: Optional[str] = None) -> ProcessResponse:
    ...
    clean_nc_path = normalize_nextcloud_inbox_path(nc_file_path, user)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Falls OCR eine neue Datei erzeugt hat und diese auf der Festplatte existiert, wieder zu Nextcloud hochladen vor dem Verschieben
        ...
        # 5. Verschieben & Umbenennen in Nextcloud per WebDAV (inkl. Kollisionsauflösung)
        try:
...
def process_webhook(request: ProcessRequest, background_tasks: BackgroundTasks):
    ...
    # Pfad normieren und auf Inbox-Ordner prüfen
    clean_path = normalize_nextcloud_inbox_path(request.file_path, request.user)
    inbox = Config.INBOX_FOLDER.strip("/")
